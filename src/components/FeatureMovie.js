...
export default function FeatureMovie({ item }) {
    ...
    return ( <
            ...
            <
            div className = "feature__points" > { item.vote_average }
            pontos < /div> {
    ...
    div className = "feature__seasons" > { item.number_of_seasons }
    temporada { item.number_of_seasons !== 1 ? "s" : "" } <
    /div> <
    div className = "feature__description" > { descr } < /div> <
    div className = "feature__buttons" >
        <
        a className = "feature__watch_button"
    href = { `/watch/${item.id}` } >
        Assistir <
        /a> <
    a className = "feature__mylist_button"
    href = { `/list/${item.id}` } >
        Minha Lista <
        /a> < /
    div > <
